...
class Solution:
    def maxConsecutiveSteps(self, arr):
        max_steps = 0
        current_steps = 0
        
        for i in range(1, len(arr)):
            if arr[i] > arr[i - 1]:  # If altitude increases
                current_steps += 1
                max_steps = max(max_steps, current_steps)
            else:
                current_steps = 0  # Reset if altitude does not increase

        return max_steps

# Test cases
sol = Solution()
print(sol.maxConsecutiveSteps([1, 2, 2, 3, 2]))  # Output: 1
print(sol.maxConsecutiveSteps([1, 2, 3, 4]))  # Output: 3
