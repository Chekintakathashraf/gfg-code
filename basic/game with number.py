...
class Solution:
    def game_with_number(self, arr, n):
        for i in range(n - 1):
            arr[i] = arr[i] ^ arr[i + 1]  # XOR consecutive elements
        return arr  # Return modified array

# Example usage:
sol = Solution()
print(sol.game_with_number([10, 11, 1, 2, 3], 5))  # Output: [1, 10, 3, 1, 3]
print(sol.game_with_number([5, 9, 7, 6], 4))  # Output: [12, 14, 1, 6]
